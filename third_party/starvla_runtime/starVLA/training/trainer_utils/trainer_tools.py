...
def build_param_lr_groups(model, cfg):
    ...
    train_patterns = _parse_module_paths(cfg.trainer.get("train_modules", ""))
    allowed_params = (
        _parameter_ids_for_module_paths(model, train_patterns, "train_modules") if train_patterns else None
    )
    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            for attr in module_name.split("."):
                module = getattr(module, attr)
            # RGB-only 配方的可选深度模块为 None，跳过对应学习率组即可。
            if module is None:
                continue
            ...
            params = [
                ...
                and id(param) not in frozen_params
                and (allowed_params is None or id(param) in allowed_params)
            ]
    ...
    other_params = [
        ...
        if param.requires_grad
        and id(param) not in used_params
        and id(param) not in frozen_params
        and (allowed_params is None or id(param) in allowed_params)
    ]
    if other_params:
    ...
    if train_patterns and not param_groups:
        raise ValueError(f"train_modules selected no optimizer parameters: {train_patterns}")
...
class TrainerUtils:
    @staticmethod
    def freeze_backbones(model, freeze_modules="", train_modules=""):
        """
        ...
        return:
          - model:
        """
        train_patterns = _parse_module_paths(train_modules)
        if train_patterns:
            trainable_ids = _parameter_ids_for_module_paths(model, train_patterns, "train_modules")
            for param in model.parameters():
                param.requires_grad_(id(param) in trainable_ids)
            if not dist.is_initialized() or dist.get_rank() == 0:
                print(f"Trainable module allowlist: {train_patterns}")
            return model
        ...
        if freeze_modules and type(freeze_modules) == str:
            ...
            patterns = _parse_module_paths(freeze_modules)
