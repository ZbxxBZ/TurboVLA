...
class LeRobotModalityMetadata(BaseModel):
    ...
    # 深度是可选模态；旧 RGB 数据集没有该字段时仍可通过 Pydantic 校验。
    depth: Optional[dict[str, LeRobotModalityField]] = Field(
        default=None,
        description="The metadata for the metric-depth modality.",
    )
    ...
    def get_key_meta(self, key: str) -> LeRobotModalityField:
        ...
        if modality == "state":
            ...
            if self.depth is None:
                raise ValueError("Depth modality is not present in this dataset metadata")
            if subkey not in self.depth:
                raise ValueError(
                    f"Key: {key}, depth key {subkey} not found in metadata, available depth keys: {self.depth.keys()}"
                )
            return self.depth[subkey]
...
class DepthMetadata(BaseModel):
    """元数据中的真实深度描述；数值单位默认是毫米。"""

    resolution: tuple[int, int] = Field(..., description="Width and height of the depth image")
    channels: int = Field(default=1, description="Number of depth channels", gt=0)
    fps: float = Field(..., description="Depth sampling rate", gt=0)
    dtype: str = Field(default="uint16", description="Stored depth dtype")
    unit: str = Field(default="millimeter", description="Stored depth unit")
...
class DatasetModalities(BaseModel):
    ...
    depth: dict[str, DepthMetadata] = Field(
        default_factory=dict,
        description="Optional metric-depth modalities",
    )
