...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 深度配方可指向独立 modality 文件，避免覆盖原 RGB-only 数据集的 meta/modality.json。
        modality_override = self.data_cfg.get("modality_metadata_path", None) if self.data_cfg else None
        self._modality_metadata_path = Path(modality_override) if modality_override else None
    ...
    def _get_modality_metadata_path(self) -> Path:
        if self._modality_metadata_path is None:
            return self.dataset_path / LE_ROBOT_MODALITY_FILENAME
        path = self._modality_metadata_path
        if not path.is_absolute():
            # 相对路径按启动训练时的仓库工作目录解析，便于直接引用 experiments 下的配方文件。
            path = Path.cwd() / path
        return path
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        # 1. Modality metadata
        modality_meta_path = self._get_modality_metadata_path()
        assert (
        ...
        # 真实深度以无损 PNG/数组保存在 parquet 中，不走有损 RGB 视频解码器。
        simplified_modality_meta["depth"] = {}
        if le_modality_meta.depth is not None:
            for new_key, depth_meta in le_modality_meta.depth.items():
                original_key = depth_meta.original_key or new_key
                if original_key not in le_info["features"]:
                    raise KeyError(f"Depth feature {original_key!r} is missing from meta/info.json")
                feature = le_info["features"][original_key]
                ...
                try:
                    ...
                    # 兼容只记录 [H, W]、未提供维度名称的自定义 LeRobot 深度字段。
                    if len(shape) < 2:
                        raise ValueError(f"Cannot infer depth resolution for feature {original_key!r}")
                    height, width = shape[-2:]
                simplified_modality_meta["depth"][new_key] = {
                    "resolution": [width, height],
                    "channels": 1,
                    "fps": float(le_info.get("fps", 30)),
                    "dtype": str(feature.get("depth_dtype", "uint16")),
                    "unit": str(feature.get("depth_unit", "millimeter")),
                }
    ...
    @staticmethod
    def _decode_depth_entry(entry, dataset_path: Path) -> np.ndarray:
        return decode_depth_entry(entry, dataset_path)

    def get_depth(self, trajectory_id: int, key: str, base_index: int) -> np.ndarray:
        """按时间索引读取真实深度，返回 [T,H,W] 且不改变原始毫米数值。"""
        step_indices = self.delta_indices[key] + base_index
        trajectory_index = self.get_trajectory_index(trajectory_id)
        step_indices = np.clip(step_indices, 0, self.trajectory_lengths[trajectory_index] - 1)
        if not key.startswith("depth."):
            raise ValueError(f"Depth key must start with 'depth.', got {key}")
        subkey = key.removeprefix("depth.")
        if self.lerobot_modality_meta.depth is None:
            raise ValueError("Dataset modality metadata does not define depth")
        original_key = self.lerobot_modality_meta.depth[subkey].original_key or subkey
        if self.curr_traj_data is None or original_key not in self.curr_traj_data.columns:
            raise KeyError(
                f"Depth column {original_key!r} is missing from trajectory {trajectory_id}; "
                "recollect/convert RoboTwin data with data_type.depth=true"
            )

        # 直接按 iloc 读取所需帧，避免每个样本都把整条 episode 的 PNG 字节列复制成 Python list。
        entries = self.curr_traj_data[original_key]
        frames = [
            self._decode_depth_entry(entries.iloc[int(index)], self.dataset_path)
    ...
    def get_data_by_modality(
        ...
        if modality == "video":
            ...
            return self.get_depth(trajectory_id, key, base_index)
